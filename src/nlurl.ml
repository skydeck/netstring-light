...
let modify_url
  ...
  let enc ?(plus=false) x =
    if encoded then
      ...
      match x with
	  None -> None
	| Some x' -> Some (Nlencoding.Url.encode ~plus x')
  in
  let enc_list ?(plus=false) l =
    if encoded then
      l
    else
      List.map (Nlencoding.Url.encode ~plus) l
  in
  ...
  let (url_host, url_port) =
    match (socksymbol : Nlsockaddr.socksymbol option) with
...
let default_url
  ...
  let encode = Nlencoding.Url.encode ~plus:true in
...
let decode_if ?(plus=false) want_encoded value =
  ...
  if want_encoded then
    ...
    Nlencoding.Url.decode ~plus value'
;;
...
let decode_path_if ?(plus=false) want_encoded value =
  if want_encoded then
    ...
    List.map (Nlencoding.Url.decode ~plus) value
;;
...
let semi_re = Nlstring_str.regexp ";";;
  ...
  let user, user_param =
	...
	None -> (None, [])
	  ...
	  if recognized url_syntax.url_enable_user_param then (
	    let l = Nlstring_str.split_delim semi_re u in
	    match l with
		[] -> (Some "", [])
...
let problem_re = Nlstring_str.regexp "[ <>\"{}|\\^`]"
let problem_hash_re = Nlstring_str.regexp "[ <>\"{}|\\^`#]"

let fixup_url_string ?(escape_hash=false) =
  Nlstring_str.global_substitute
    (if escape_hash then problem_hash_re else problem_re)
    (fun m s ->
       sprintf "%%%02x" (Char.code s.[Nlstring_str.match_beginning m]))
;;
...
let backslash_re = Nlstring_str.regexp "\\\\";;
let drive_letter_re = Nlstring_str.regexp "^\\([A-Za-z]\\):/";;
let drive_letter_re' = Nlstring_str.regexp "^/\\([A-Za-z]\\):/";;
let unc_path_re = Nlstring_str.regexp "^//\\([^/]+\\)\\(/|$\\)";;
...
let classify_path p =
  match os_type with
	...
	if p <> "" && p.[0] = '/' then
	  ...
	  `Relative p
    | "Win32" ->
	let p' = Nlstring_str.global_replace backslash_re "/" p in
	( match Nlstring_str.string_match drive_letter_re p' 0 with
	      Some m ->
		`Absolute_local ("/" ^ p')
	    | None ->
		( match Nlstring_str.string_match unc_path_re p' 0 with
		      Some m ->
			let host = Nlstring_str.matched_group m 1 p' in
			let host_e = Nlstring_str.group_end m 1 in
	...
	( match Nlstring_str.string_match drive_letter_re p' 0 with
	      Some m ->
		let letter = Nlstring_str.matched_group m 1 p' in
...
let file_url_of_local_path ?(getcwd = Sys.getcwd) p =
  ...
  let p_abs_class =
	...
	`Relative r ->
	  ...
	  ( match classify_path (getcwd()) with
		`Absolute_local l ->
		  ( match Nlstring_str.string_match drive_letter_re' l 0 with
			Some m ->
			  let letter = Nlstring_str.matched_group m 1 l in
...
let drive_letter_comp_re = Nlstring_str.regexp "^\\([A-Za-z]\\)\\(:|\\|\\)";;
...
let local_path_of_file_url u =
  let local_path p =
	...
	"Unix" ->
	  ...
	  ( match p with
		("" :: drive :: rest) ->
		  (match Nlstring_str.string_match drive_letter_comp_re drive 0 with
		       Some m ->
			 let letter = Nlstring_str.matched_group m 1 drive in
